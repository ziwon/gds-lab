...
void pread_exact(int fd, void* buffer, std::size_t bytes, off_t offset) {
    ...
    while (done < bytes) {
        ...
        if (n == 0) throw std::runtime_error("unexpected EOF: file is smaller than --offset + --bytes");
...
BenchmarkResult run_overlap(const BenchmarkOptions& options) {
    ...
    if (options.direct) {
        if ((chunk_bytes % kDirectAlignment) != 0 || (options.offset % kDirectAlignment) != 0 ||
            (options.bytes % kDirectAlignment) != 0) {
            throw std::runtime_error(
                "SKIPPED: --bytes, --offset and --chunk-bytes must be multiples of 4096 for O_DIRECT");
        }
    }

    const int flags = options.direct ? (O_RDONLY | O_DIRECT) : O_RDONLY;
    const int raw_fd = ::open(options.path.c_str(), flags);
    ...
    try {
        for (int slot = 0; slot < 2; ++slot) {
            ...
            // O_DIRECT needs an aligned user buffer. cudaHostAlloc is page-aligned
            // in practice; verify instead of assuming.
            if (options.direct &&
                (reinterpret_cast<std::uintptr_t>(host[slot]) % kDirectAlignment) != 0) {
                throw std::runtime_error("pinned buffer is not 4 KiB aligned; O_DIRECT unusable");
            }
            cuda_check(cudaMalloc(reinterpret_cast<void**>(&device[slot]), chunk_bytes), "cudaMalloc");
            cuda_check(cudaEventCreateWithFlags(&copy_done[slot], cudaEventDisableTiming),
                       "cudaEventCreate");
        ...
        // Chunk visit order. Shuffling only changes which file extent each
        // pipeline step reads; the checksum is a sum, so it stays identical.
        const std::size_t steps = (options.bytes + chunk_bytes - 1) / chunk_bytes;
        std::vector<std::size_t> order(steps);
        ...
        for (std::size_t step = 0; step < steps; ++step) {
            ...
            if (step >= 2) {
                cuda_check(cudaEventSynchronize(compute_done[slot]), "cudaEventSynchronize(reuse)");
            }

            const std::size_t block_offset = order[step] * chunk_bytes;
            const std::size_t current = std::min(chunk_bytes, options.bytes - block_offset);
            pread_exact(fd.fd, host[slot], current,
                        static_cast<off_t>(options.offset + block_offset));

            cuda_check(cudaMemcpyAsync(device[slot], host[slot], current, cudaMemcpyHostToDevice,
                                       copy_stream), "cudaMemcpyAsync");
        ...
        result.label = std::string("double-buffer-overlap") +
                       (options.direct ? "-odirect" : "") +
                       (options.random_access ? "-random" : "");
        result.bytes = processed;
        result.seconds = std::chrono::duration<double>(end - start).count();
        // Read and copy are pipelined on purpose; splitting them here would
        // report overlapped time twice.
        result.split_valid = false;
        result.checksum = static_cast<std::uint64_t>(checksum);
